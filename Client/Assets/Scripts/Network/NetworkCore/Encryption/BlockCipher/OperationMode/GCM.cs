    ...
    public class GCM : OperationMode
    {
        ...
        private byte[] GFMultiply(byte[] x, byte[] y)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                {
                    // if (((temp >>> (7 - j)) & 0x01) == 1)
                    if ((UnsignedRightShift(temp, 7 - j) & 0x01) == 1)
                    {
                        for (int k = 0; k < 16; k++)
                        {
                            z[k] ^= v[k];
        ...
        private byte[] RightShift(byte[] v)
        {
            ...
            // v[15] = (byte)(v[15] >>> 1);
            v[15] = UnsignedRightShift(v[15], 1);
            for (int i = 14; i >= 0; i--)
            {
                if ((v[i] & 0x01) == 1)
                {
        ...
        private static byte UnsignedRightShift(byte a, int shift)
        {
            return (byte)((a & 0xFF) >> shift);
        }
