#ifndef CPRACTICE_ARIA_H
#define CPRACTICE_ARIA_H

#include <iostream>
#include <cstdint>
#include <cstring>
#include <iomanip>

class ARIA {
public:
    // Diffusion Layer
    void DiffusionLayer(const uint8_t* i, uint8_t* o) {
        uint8_t T;

        T = i[ 3] ^ i[ 4] ^ i[ 9] ^ i[14];
        o[ 0] = i[ 6] ^ i[ 8] ^ i[13] ^ T;
        o[ 5] = i[ 1] ^ i[10] ^ i[15] ^ T;
        o[11] = i[ 2] ^ i[ 7] ^ i[12] ^ T;
        o[14] = i[ 0] ^ i[ 5] ^ i[11] ^ T;
        T = i[ 2] ^ i[ 5] ^ i[ 8] ^ i[15];
        o[ 1] = i[ 7] ^ i[ 9] ^ i[12] ^ T;
        o[ 4] = i[ 0] ^ i[11] ^ i[14] ^ T;
        o[10] = i[ 3] ^ i[ 6] ^ i[13] ^ T;
        o[15] = i[ 1] ^ i[ 4] ^ i[10] ^ T;
        T = i[ 1] ^ i[ 6] ^ i[11] ^ i[12];
        o[ 2] = i[ 4] ^ i[10] ^ i[15] ^ T;
        o[ 7] = i[ 3] ^ i[ 8] ^ i[13] ^ T;
        o[ 9] = i[ 0] ^ i[ 5] ^ i[14] ^ T;
        o[12] = i[ 2] ^ i[ 7] ^ i[ 9] ^ T;
        T = i[ 0] ^ i[ 7] ^ i[10] ^ i[13];
        o[ 3] = i[ 5] ^ i[11] ^ i[14] ^ T;
        o[ 6] = i[ 2] ^ i[ 9] ^ i[12] ^ T;
        o[ 8] = i[ 1] ^ i[ 4] ^ i[15] ^ T;
        o[13] = i[ 3] ^ i[ 6] ^ i[ 8] ^ T;
    }
    // Right-rotate 128 bit source string s by n bits and XOR it to target string t
    void RotXOR(const uint8_t* s, int n, uint8_t* t) {
        int i, q;

        q = n/8; n %= 8;
        for (i = 0; i < 16; i++) {
            t[(q+i) % 16] ^= (s[i] >> n);
            if (n != 0) t[(q+i+1) % 16] ^= (s[i] << (8-n));
        }
    }
    // Encryption round key generation rountine
    // w0 : master key, e : encryption round keys
    int EncKeySetup(const uint8_t* w0, uint8_t* e, int keyBits) {
        int  i, R=(keyBits+256)/32, q;
        uint8_t t[16], w1[16], w2[16], w3[16];

        q = (keyBits - 128) / 64;
        for (i = 0; i < 16; i++) t[i] = sbox[     i  % 4][rcon[q][i   ] ^ w0[i]];
        DiffusionLayer(t, w1);
        if (R==14)
            for (i = 0; i <  8; i++) w1[i] ^= w0[16+i];
        else if (R==16)
            for (i = 0; i < 16; i++) w1[i] ^= w0[16+i];

        q = (q==2)? 0 : (q+1);
        for (i = 0; i < 16; i++) t[i] = sbox[(2 + i) % 4][rcon[q][i] ^ w1[i]];
        DiffusionLayer (t, w2);
        for (i = 0; i < 16; i++) w2[i] ^= w0[i];

        q = (q==2)? 0 : (q+1);
        for (i = 0; i < 16; i++) t[i] = sbox[     i  % 4][rcon[q][i] ^ w2[i]];
        DiffusionLayer (t, w3);
        for (i = 0; i < 16; i++) w3[i] ^= w1[i];

        for (i = 0; i < 16*(R+1); i++) e[i] = 0;

        RotXOR (w0, 0, e      ); RotXOR (w1,  19, e      );
        RotXOR (w1, 0, e +  16); RotXOR (w2,  19, e +  16);
        RotXOR (w2, 0, e +  32); RotXOR (w3,  19, e +  32);
        RotXOR (w3, 0, e +  48); RotXOR (w0,  19, e +  48);
        RotXOR (w0, 0, e +  64); RotXOR (w1,  31, e +  64);
        RotXOR (w1, 0, e +  80); RotXOR (w2,  31, e +  80);
        RotXOR (w2, 0, e +  96); RotXOR (w3,  31, e +  96);
        RotXOR (w3, 0, e + 112); RotXOR (w0,  31, e + 112);
        RotXOR (w0, 0, e + 128); RotXOR (w1,  67, e + 128);
        RotXOR (w1, 0, e + 144); RotXOR (w2,  67, e + 144);
        RotXOR (w2, 0, e + 160); RotXOR (w3,  67, e + 160);
        RotXOR (w3, 0, e + 176); RotXOR (w0,  67, e + 176);
        RotXOR (w0, 0, e + 192); RotXOR (w1,  97, e + 192);
        if (R > 12) {
            RotXOR (w1, 0, e + 208); RotXOR (w2,  97, e + 208);
            RotXOR (w2, 0, e + 224); RotXOR (w3,  97, e + 224);
        }
        if (R > 14) {
            RotXOR (w3, 0, e + 240); RotXOR (w0,  97, e + 240);
            RotXOR (w0, 0, e + 256); RotXOR (w1, 109, e + 256);
        }
        return R;
    }
    // Decryption round key generation rountine
    // w0 : maskter key, d : decryption round keys
    int DecKeySetup(const uint8_t* w0, uint8_t* d, int keyBits) {
        int  i, j, R;
        uint8_t t[16];

        R=EncKeySetup(w0, d, keyBits);
        for (j = 0; j < 16; j++){
            t[j] = d[j];
            d[j] = d[16*R + j];
            d[16*R + j] = t[j];
        }
        for (i = 1; i <= R/2; i++){
            DiffusionLayer (d + i*16, t);
            DiffusionLayer (d + (R-i)*16, d + i*16);
            for (j = 0; j < 16; j++) d[(R-i)*16 + j] = t[j];
        }
        return R;
    }
    // Encryption and decryption rountine
    // p: plain text, e: round keys, c: ciphertext
    void Crypt(const uint8_t* p, int R, const uint8_t* e, uint8_t* c) {
        int i, j;
        uint8_t t[16];

        for (j = 0; j < 16; j++) c[j] = p[j];
        for (i = 0; i < R/2; i++)
        {
            for (j = 0; j < 16; j++) t[j] = sbox[     j  % 4][e[j] ^ c[j]];
            DiffusionLayer(t, c); e += 16;
            for (j = 0; j < 16; j++) t[j] = sbox[(2 + j) % 4][e[j] ^ c[j]];
            DiffusionLayer(t, c); e += 16;
        }

        DiffusionLayer(c, t);
        for (j = 0; j < 16; j++) c[j] = e[j] ^ t[j];
    }
    void printBlockOfLength(const uint8_t* b, int len) {
        int i;

        for (i=0; i<len; i++, b++) {
            printf("%02x", *b);
            if (i%4==3 && i<len-1) printf(" ");
        }
    }
    void printBlock(const uint8_t* b) {
        printBlockOfLength(b, 16);
    }
    void test() {
        uint8_t rk[16 * 17], c[16], mk[16];
        uint8_t p[16]={0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa,
                    0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb};
        const uint8_t cryptResult[16] = {
                0xc6, 0xec, 0xd0, 0x8e, 0x22, 0xc3, 0x0a, 0xbd,
                0xb2,0x15, 0xcf, 0x74, 0xe2, 0x07, 0x5e, 0x6e
        };
        for (int i=0; i<16; i++) mk[i] = i * 0x11;
        Crypt(p, EncKeySetup(mk, rk, 128), rk, c);
        printf("BEGIN testing basic encryption...\n");
        printf("Testing whether the encryption would come out correctly, for 12-round ARIA.\n");
        printf("key      : "); printBlockOfLength(mk, 24); printf("\n");
        printf("plaintext: "); printBlock(p); printf("\n");
        printf("result is: "); printBlock(c); printf("\n");
        printf("should be: "); printBlock((uint8_t *)cryptResult); printf("\n");
        int flag=0;
        for (int i=0; i<16; i++) {
            if (c[i]!=cryptResult[i])
                flag=1;
        }
        if (flag==1) {
            printf("The result is incorrect!\n");
        }
        else {
            printf("Okay.  The result is correct.\n");
        }
        printf("END   testing basic encryption.\n\n");

        for (int i=0; i<16; i++) p[i]=0;
        for (int i=0; i<16; i++) mk[i]=0;
        printf("plaintext : "); printBlock(p); printf("\n");
        Crypt(p, EncKeySetup(mk, rk, 128), rk, c);
        printf("ciphertext: "); printBlock(c); printf("\n");
        Crypt(c, DecKeySetup(mk, rk, 128), rk, p);
        printf("decrypted : "); printBlock(p); printf("\n");
    }
private:
    /* --------------
    |    constant
    ---------------*/
    const uint8_t sbox[4][256] = {
            // S-box type 1
            {
                    },
            // S-box type 2
            {
                    },
            // inverse of S-box type 1
            {
                    },
            // inverse of S-box type 2
            {
                    }
    };
    const uint8_t rcon[3][16] = {
            {0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0},
            {0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0},
            {0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e}
    };
};


#endif //CPRACTICE_ARIA_H
